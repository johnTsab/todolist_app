...
//get tasks by user (get the full list of tasks of a user along with each tasks subtask)
...
const deleteTaskofUser = async(req,res) => {
    ...
    try{
        ...
        const logAction = `ADMIN DELETED TASK: ${taskInfo} FROM USER: ${userId}`;
        await db.query(
              "INSERT INTO logs (user_id,action,activity_type) VALUES (?,?,?)",
...
const getSubtasksofUser = async(req,res) =>{
    ...
    const taskId = Number(req.params.taskid);
    try{
        const [rows] = await db.query("SELECT * FROM subtasks WHERE id=? AND user_id=?",[])
...
const updateSubtaskofUser = async (req,res) => {
    ...
    const taskId = req.params.taskid;
    try{
        const [result] = await db.query('UPDATE tasks WHERE ')
...
module.exports={getAllUsers,deleteUser,getTasksOfUser,updateTaskofUser,deleteTaskofUser}